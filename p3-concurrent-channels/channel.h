...
typedef struct {
    ...
    // we are told to use buffer_create function
    buffer_t* buffer;
    
    //Store the semaphore for send operation.
    list_t* recv_list;
    //Store the semaphore for recieve operation.
    list_t* send_list;
    
    /* ADD ANY STRUCT ENTRIES YOU NEED HERE */
    /* IMPLEMENT THIS */
    
    // flag closed is needed 
    bool closed; 
    
    // instead of using mutex/cond vars, we are using semaphores
    // semaphore functioning as mutex
    sem_t mutex;
    // need cond var for availabe slots (for send/recv)
    sem_t available;
    // also need cond var for used slots (for send/recv)
    sem_t used;
    
    //mutex to synchronize recv list.
    pthread_mutex_t recv_lock;
    //mutex to synchronize send list.
